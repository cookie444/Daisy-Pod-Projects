...
// Tuning
// ---------------------------------------------------------------------------
static constexpr int   kNumModes        = 16;     // resonators in the bank
...
// Modes are spread over this range so every string finds a partial
static constexpr float kSpanSemis       = 36.0f;
...
static void BuildFreqs()
{
    ...
    {
        float target = kSpanSemis * (float)i / (float)(kNumModes - 1);
        int   semis  = SnapToScale(p, target);
        if(semis <= prev)
            semis = prev + 1;
        prev    = semis;
                   ...
                   size_t                                size)
{
    ...
    // Button 1 -> freeze: gate the excitation and stretch the decay out
    bool new_freeze = pod.button1.Pressed();
