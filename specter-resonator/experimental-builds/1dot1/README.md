# Specter Resonator — build 1.1 (experimental)

A resonant bank for the **Electrosmith Daisy Pod**, built on libDaisy and DaisySP.

Your playing excites sixteen tuned resonators instead of going straight to the output, so anything you play comes back as a chord built from the selected scale. Hold the freeze button and the current ring is held indefinitely while your live signal drops away — a drone you can then strike, bow, or play over.
...
| Button 1 | FREEZE — hold to gate the excitation and stretch the decay to 60 s |
...
Turning the encoder walks the bank through these tunings, all rooted on A:
...
- The partials are spread evenly over three octaves and snapped to the nearest scale degree, so every string finds something to excite rather than only the low ones. Each is detuned a few cents so exact harmonic alignments don't become hot spots.
...
- The sixteen partials are spread across the stereo field, low on the left, and the sum runs through a tanh soft clip.
...
Produces `build/specter-resonator-1dot1.bin`.
...
|----------|---------|---------|
| `kNumModes` | `16` | Resonators in the bank |
...
| `kSpanSemis` | `36` | Range the modes are spread across |
