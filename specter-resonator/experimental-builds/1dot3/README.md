# Specter Resonator — build 1.3 (experimental)
...
| Button 1 | Tap to step the tuning. Hold past ~350 ms to FREEZE: gates the excitation and holds the ring |
| Button 2 | STRIKE — press for a noise burst into the bank, hold to bow it with noise |
| Encoder turn | Key — steps a semitone per detent, up or down |
...
- **Encoder turn** steps the key — 12 roots, A through G#, one semitone per detent, up or down. LED 1 blips when it changes. The tuning shape stays put and the whole bank transposes.
- **Button 1 tap** (shorter than ~350 ms) steps the tuning. Hold it longer and it freezes instead.

The dial is the only control that goes both directions, so it gets the key — that way stepping a semitone down is as easy as stepping up.
...
Produces `build/specter-resonator-1dot3.bin`.
